...
• Не давай медицинских, юридических или психиатрических диагнозов. При тревоге, депрессии, насилии — мягко предлагай обратиться к специалисту.
• Никогда не стыди и не обвиняй. Даже жёсткую правду подавай через «здесь важно учесть…», «стоит обратить внимание…».

**КАЧЕСТВО ОТВЕТОВ (ПРЕМИУМ-УРОВЕНЬ)**
• Сначала — короткое отражение ситуации («Понимаю, вам сейчас непросто с…»), чтобы человек почувствовал, что его услышали.
• Затем — 2–4 конкретных, применимых шага или идеи. Без воды и общих фраз вроде «нужно поговорить» без расшифровки как именно.
• В конце — одно резюме или один следующий шаг. Можно предложить: «Если хотите, разберём подробнее [конкретную тему]».
• Для простых вопросов — лаконично, по делу. Для сложных — можно развернуто, но структурированно: блоки с **подзаголовками**, списки через • или -.
• Не обрывай мысль на полуслове. Если лимит символов подходит к концу — сократи середину, но начало и конец (вывод) оставь завершёнными.

**ФОРМАТИРОВАНИЕ**
• Не используй символ #. Заголовки — **жирный текст**.
• Списки — маркеры • или -. Нумерация только если реально нужна (шаги по порядку), не более 5 пунктов.
• Каждый абзац и список завершай полным предложением. Проверь перед отправкой, что ответ логически закончен.
...
• Адаптируй тон под сообщение: больше тепла при грусти/страхе, больше чёткости при запросе плана действий.
• Эмодзи — умеренно, для мягкости, не в каждом предложении.

${instructions ? 'ДОПОЛНИТЕЛЬНАЯ БАЗА ЗНАНИЙ (используй при ответах):\n' + instructions : ''}

**ПРИМЕР ПРЕМИУМ-ОТВЕТА**
**Понимаю вашу ситуацию** — когда хочется писать первой, а вы уже решили держать паузу, это нормально чувствовать тревогу.

Важно помнить:
• Каждый ваш контакт без её инициативы — это сигнал, что границы можно снова сдвигать. Тишина с вашей стороны как раз и даёт пространство для её мыслей.
• Перенаправьте фокус: работа, спорт, встречи с друзьями, хобби. Цель — не «наказать» её, а вернуть себе опору в себе самом.
• Если через 2–4 недели тишины с её стороны не будет — можно один раз спокойно спросить, как у неё дела, без требований и сцен.

**Резюме:** Сейчас самый сильный шаг — не писать. Хотите, разберём, как справляться с желанием написать в моменте?`
        };
    }
...
calculateDynamicMaxTokens(message, userAnalysis) {
    ...
    // 1. Базовый анализ длины
    const wordCount = message.split(/\s+/).length;
    
    // 2. Расширенный анализ сложности для психологических тем
    const messageLower = message.toLowerCase();
    
    // Категории вопросов, требующие больше токенов
    const categories = {
        // Ситуации, требующие развернутого анализа
        complexSituations: [
            'отношения', 'любовь', 'брак', 'расставание', 'развод',
            'измена', 'ссора', 'конфликт', 'кризис', 'проблема',
            'ситуация', 'случилось', 'произошло'
        ],
        // Запросы на развернутый ответ
        detailedRequests: [
            'подробно', 'детально', 'развернуто', 'объясни',
            'расскажи', 'посоветуй', 'помоги', 'что делать',
            'как быть', 'как понять', 'стоит ли'
        ],
        // Эмоционально заряженные темы (требуют эмпатии = больше токенов)
        emotionalTopics: [
            'больно', 'тяжело', 'сложно', 'страшно', 'тревожно',
            'обидно', 'одиноко', 'депрессия', 'устал', 'страдаю'
        ],
        // Манипуляции и абьюз (требуют осторожных формулировок)
        manipulationTopics: [
            'манипуляция', 'абьюз', 'токсичный', 'газлайтинг',
            'контроль', 'унижение', 'оскорбление'
        ]
    };
    
    // Подсчет баллов по каждой категории
    let categoryScore = 0;
    for (const [category, keywords] of Object.entries(categories)) {
        const matches = keywords.filter(kw => messageLower.includes(kw)).length;
        if (matches > 0) {
            categoryScore += matches;
            console.log(`📊 Категория "${category}": +${matches} баллов`);
        }
    }
    
    // 3. Анализ структуры вопроса
    const hasQuestionMark = message.includes('?');
    const hasExclamationMark = message.includes('!');
    const sentenceCount = message.split(/[.!?]+/).filter(s => s.trim().length > 0).length;
    
    // 4. Определяем базовый множитель по длине (более щадящий)
    let multiplier = 1.0; // По умолчанию 1000 токенов
    ...
    if (categoryScore > 0) {
        // Каждый балл категории добавляет 0.1 к множителю
        // Но не более +0.8
        multiplier += Math.min(0.8, categoryScore * 0.1);
    }
    
    // 6. Учитываем эмоциональное состояние
    if (userAnalysis.emotionalState === 'negative') {
        // Негативный фон требует больше эмпатии
        multiplier += 0.2;
    }
    if (userAnalysis.isHighEnergy) {
        // Высокая энергия = срочность = нужен полный ответ
        multiplier += 0.2;
    }
    
    // 7. Учитываем структуру вопроса
    if (hasQuestionMark && sentenceCount > 2) {
        // Несколько предложений с вопросом = сложный запрос
        multiplier += 0.2;
    }
    
    // 8. ВСЕГДА даем минимум 800 токенов для любых вопросов
    const minTokens = 800;
    
    // Вычисляем финальное значение
    let calculatedTokens = Math.round(baseTokens * multiplier);
    
    // Применяем минимум
    calculatedTokens = Math.max(minTokens, calculatedTokens);
    
    // Ограничиваем максимум (но можно увеличить, если нужно)
    calculatedTokens = Math.min(2500, calculatedTokens);
    
    // Округляем до ближайшего "красивого" числа (кратного 100)
    const roundedTokens = Math.round(calculatedTokens / 100) * 100;
    
    console.log(`📊 Динамический max_tokens:`, {
        слов: wordCount,
        категорий: categoryScore,
        эмоции: userAnalysis.emotionalState,
        множитель: multiplier.toFixed(2),
        токенов: roundedTokens
    });
