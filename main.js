...
export class VerdiktChatApp {
    startStarSuctionEffect() {
    const starContainer = document.getElementById('logo-star-suction');
    ...
    const createStar = () => {
        ...
        // Генерируем случайную позицию на краю контейнера
        const angle = Math.random() * Math.PI * 2;
        const distance = 200 + Math.random() * 150; // Расстояние от центра
        ...
        star.style.animation = `starSuction ${duration}s ease-out forwards`;
    ...
    // Создаем звезды с высокой частотой в начале
    for (let i = 0; i < 20; i++) {
        setTimeout(() => createStar(), i * 50);
    }
    
    // Затем продолжаем с меньшей частотой
    this.starSuctionInterval = setInterval(createStar, 300);
}

stopStarSuctionEffect() {
    if (this.starSuctionInterval) {
        clearInterval(this.starSuctionInterval);
